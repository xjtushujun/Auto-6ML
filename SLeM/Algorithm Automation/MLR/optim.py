...
class Optimizer(object):
    ...
    def pre_step(self, loss):
        ...
        for pg in self.param_groups:
            if "grads" not in pg:
                pg["grads"] = [ jt.zeros_like(p).stop_fuse() for p in pg['params'] ]
            pg_grads = pg["grads"]
...
class SGD(Optimizer):
    ...
    def __init__(self, params, lr, momentum=0, weight_decay=0, dampening=0, nesterov=False):
        ...
        for pg in self.param_groups:
            values = pg["values"] = []
            for p in pg["params"]:
                values.append(jt.zeros(p.shape, p.dtype))
    ...
    def step(self, lr=None, loss=None):
        if loss is not None:
            self.pre_step(loss)
        for pg in self.param_groups:
...
class Adam(Optimizer):
    ...
    def __init__(self, params, lr, eps=1e-8, betas=(0.9, 0.999), weight_decay=0):
        ...
        for pg in self.param_groups:
            ...
            for p in pg["params"]:
                values.append(jt.zeros(p.shape, p.dtype))
                m.append(jt.zeros(p.shape, p.dtype))
    ...
    def step(self, loss=None):
        ...
        for pg in self.param_groups:
            ...
            for p, g, v, m in zip(pg["params"], pg["grads"], pg["values"], pg["m"]):
                ...
                p.assign(p - m * step_size / (jt.sqrt(v) + eps))
        self.zero_grad()
