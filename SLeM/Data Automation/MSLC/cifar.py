...
class CIFAR10(Dataset):
    ...
    def __getitem__(self, index):
        # print('self.train:', self.train, self.meta, self.train is True, self.meta is True)
...
def build_dataset(args):
    if args.dataset == 'cifar10':
        ...
        train_meta_loader = CIFAR10(
            ...
            corruption_type=args.corruption_type, transform=train_transform, download=True).set_attrs(num_workers=args.prefetch, batch_size=args.batch_size,shuffle=True)
        train_loader = CIFAR10(
            root='../data', train=True, meta=False, num_meta=args.num_meta, corruption_prob=args.corruption_prob,
            corruption_type=args.corruption_type, transform=TransformTwice(train_transform), download=True, seed=args.seed).set_attrs(num_workers=args.prefetch, batch_size=args.batch_size,shuffle=True)
        ...
        train_loader = CIFAR100(
            root='../data', train=True, meta=False, num_meta=args.num_meta, corruption_prob=args.corruption_prob,
            corruption_type=args.corruption_type, transform=train_transform, download=True, seed=args.seed).set_attrs(num_workers=args.prefetch, batch_size=args.batch_size,shuffle=True)
