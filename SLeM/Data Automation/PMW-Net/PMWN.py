...
def beta_sample(alpha, beta, size):
    # Sample from Gamma distribution
    v_lambda = jt.array([])
    for i in range(len(alpha)):
        x = jt.random.gamma(alpha[i, 0], 1.0, [1,])
        y = jt.random.gamma(beta[i, 0], 1.0, [1,])
        v_lambda = jt.cat([v_lambda, x / (x + y) ])

    return v_lambda


def train(train_loader,train_meta_loader, model, vnet_alpha, vnet_beta,optimizer_model, optimizer_vnet_alpha, optimizer_vnet_beta,epoch, meta_lr):
    print('\nEpoch: %d' % epoch)
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        alpha = vnet_alpha(cost.detach())
        beta = vnet_beta(cost.detach())
        v_post = beta_sample(alpha, beta)
        v_lambda = v_post.reshape((-1,1))
        ...
        optimizer_vnet_alpha.step(l_g_meta, retain_graph=True)
        optimizer_vnet_beta.step(l_g_meta)
...
# load model
model = build_model()
vnet_alpha = VNet(1, 100, 1)
vnet_beta = VNet(1, 100, 1)

optimizer_model = jt.optim.SGD(model.parameters(), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
optimizer_vnet_alpha = jt.optim.Adam(vnet_alpha.parameters(), 1e-3, weight_decay=1e-4)
optimizer_vnet_beta = jt.optim.Adam(vnet_beta.parameters(), 1e-3, weight_decay=1e-4)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader,train_meta_loader,model, vnet_alpha, optimizer_model, vnet_beta, optimizer_vnet_alpha, optimizer_vnet_beta,epoch, meta_lr)
