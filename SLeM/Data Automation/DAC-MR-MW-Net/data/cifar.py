...
def build_dataset(args):
    if args.dataset == 'cifar10':
        ...
        transform_strong = transform.Compose([
            transform.ToTensor(),
            transform.Lambda(lambda x: (np.pad( x, ((0,0), (4,4), (4,4)), mode='reflect')).transpose(1,2,0)),
            transform.ToPILImage(),
            transform.RandomCrop(32),
        ...
        transform_strong.transforms.append(CutoutDefault(8))

        train_meta_loader = CIFAR10(
            root='../data', train=True, meta=True, num_meta=args.num_meta, corruption_prob=args.corruption_prob,
            corruption_type=args.corruption_type, transform=TransformTwice(train_transform, transform_strong), download=True).set_attrs(num_workers=args.prefetch, batch_size=args.batch_size,shuffle=True)
