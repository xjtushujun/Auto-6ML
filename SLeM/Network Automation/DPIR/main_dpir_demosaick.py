...
def main():
    ...
    for idx, img in enumerate(L_paths):
        ...
        if matlab_init:  # matlab demosaicing for initialization
            CFA4 = util.uint2tensor4(CFA4)
            x = utils_mosaic.dm_matlab(CFA4)
        else:
            x = cv2.cvtColor(CFA, cv2.COLOR_BAYER_BG2RGB_EA)
            x = util.uint2tensor4(x)

        img_L = util.tensor2uint(x)
        y = util.uint2tensor4(mosaic)

        util.imshow(img_L) if show_img else None
        mask = util.single2tensor4(mask.astype(np.float32))
        ...
        rhos, sigmas = torch.tensor(rhos), torch.tensor(sigmas)
