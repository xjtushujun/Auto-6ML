...
def c2c(x):
    return (np.stack([np.float32(x.real), np.float32(x.imag)], axis=-1))
...
def rfft(t):
    ...
    return FFT.rfft(t, 2, onesided=False)


def irfft(t):
    # Complex-to-real Inverse Discrete Fourier Transform
    return FFT.irfft(t, 2, onesided=False)
...
def p2o(psf, shape):
    ...
    otf = FFT.rfft(otf, 2, onesided=False)
...
def data_solution(x, FB, FBC, F2B, FBFy, alpha, sf):
    FR = FBFy + FFT.rfft(alpha*x, 2, onesided=False)
    x1 = cmul(FB, FR)
    ...
    Xest = FFT.irfft(FX, 2, onesided=False)
    return Xest
...
def pre_calculate(x, k, sf):
    ...
    FBFy = cmul(FBC, FFT.rfft(STy, 2, onesided=False))
...
def imfilter(x, k):
    ...
    x = nn.Conv(x, k, groups=x.shape[1])
    return x
