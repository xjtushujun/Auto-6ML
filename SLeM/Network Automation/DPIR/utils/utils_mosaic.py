...
def dm(imgs):
    ...
    Returns:
        output: Nx3xWxH
    """
    k_r = 1/4 * torch.Tensor([[1, 2, 1], [2, 4, 2], [1, 2, 1]]).type_as(imgs)
    k_g = 1/4 * torch.Tensor([[0, 1, 0], [1, 4, 1], [0, 1, 0]]).type_as(imgs)
    ...
    rgb = nn.pad(rgb, (1, 1, 1, 1), mode='circular')
    rgb = np.convolve(rgb, k)
...
def dm_matlab(imgs):
    ...
    kgrb = 1/8*torch.Tensor([[0, 0, -1, 0, 0],
                                    [0, 0, 2, 0, 0],
                                    [-1, 2, 4, 2, -1],
                                    [0, 0, 2, 0, 0],
                                    [0, 0, -1, 0, 0]]).type_as(imgs)
    krbg0 = 1/8*torch.Tensor([[0, 0, 1/2, 0, 0],
                                      [0, -1, 0, -1, 0],
                                      [-1, 4, 5, 4, -1],
                                      [0, -1, 0, -1, 0],
                                      [0, 0, 1/2, 0, 0]]).type_as(imgs)
    krbg1 = krbg0.t()
    krbbr = 1/8*torch.Tensor([[0, 0, -3/2, 0, 0],
                                     [0, 2, 0, 2, 0],
                                     [-3/2, 0, 6, 0, -3/2],
                                     [0, 2, 0, 2, 0],
